...
| 指標名稱 | 定義 | 值域 / 方向 | 解讀重點 |
|---|---|---|---|
| **Top-1 Accuracy** | 正確匹配的候選物件是否排在第 1 名：<br>Top1 = (1 / Qv) * Σ I[argmax_j P(i,j) = gt_i] | [0,1]，**越大越好** | 最直觀的辨識能力指標，代表模型「第一個選擇」是否正確 |
| **Mean Rank** | 正確目標在排序中的平均名次：<br>MeanRank = (1 / Qv) * Σ rank_i | [1, N]，**越小越好** | 模型通常把正確目標排在第幾名，越靠前代表區分能力越好 |
| **MRR (Mean Reciprocal Rank)** | 名次倒數的平均值：<br>MRR = (1 / Qv) * Σ (1 / rank_i) | (0,1]，**越大越好** | 強調前幾名的重要性，rank=1 影響最大 |
| **Recall@K** | 正確目標是否落在前 K 名：<br>Recall@K = (1 / Qv) * Σ I[rank_i ≤ K] | [0,1]，**越大越好** | 模型在 Top-K 內是否能「找得到正解」 |
